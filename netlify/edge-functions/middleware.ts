...
function parseCookie(header: string | null, name: string): string | null {
  ...
  for (const part of header.split(/;\s*/)) { const [k,v] = part.split("="); if (k === name) return decodeURIComponent(v || ""); }
  return null;
}
...
function isIpBanned(ip: string, entries: Array<{ pattern: string }>): boolean {
  ...
  for (const e of entries || []) {
    const p = e.pattern;
    if (!p) continue;
    if (p.includes("/")) { if (inCIDR(ip, p)) return true; }
    else { if (ip === p) return true; }
  }
  return false;
}
...
export default async (request: Request, context: any) => {
  ...
  //    c) Static assets (don’t waste Edge time on CSS/JS/images)
  ...
  try {
    ...
    // If bans fetch fails, fail-open (allow request) to avoid lockout
  }

  // 3) Admin guard only on /admin
