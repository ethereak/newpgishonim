...
exports.handler = async (event) => {
  try {
    ...
    const siteUrl   = (process.env.SITE_URL || "").replace(/\/+$/, ""); // <-- trim
      ...
      body: JSON.stringify({
        data: {
          ...
          attributes: {
            checkout_data: {
              email,
              custom: { token, email, student_name, class: klass, date, release_time, reason, approved_by }
            },
    ...
    if (!url) return { statusCode: 500, body: JSON.stringify({ error: "no_checkout_url" }) };

    return { statusCode: 200, body: JSON.stringify({ url }) };
  } catch (e) {
    return { statusCode: 500, body: JS
